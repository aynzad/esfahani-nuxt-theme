...
const name = computed(() => (isRtl.value ? 'علیرضا اصفهانی' : 'Alireza Esfahani'))
</script>
  ...
  >
    ...
    <div style="display: flex; justify-content: center; width: 100%;">
      <div style="position: relative; display: flex; align-items: center; justify-content: center;">
        ...
        <span style="font-size: 24px; font-weight: 700; color: #c62641; white-space: nowrap;">{{ name }}</span>
      </div>
    </div>

    <!-- Page title + description (left-aligned body, as before). -->
    <div style="display: flex; flex-direction: column;">
      <div style="color: #c62641; font-size: 72px; font-weight: 700; line-height: 1.1;">
        {{ title }}
      </div>
      <div
        v-if="description"
        style="color: #2b4338; font-size: 34px; margin-top: 24px; line-height: 1.4;"
      >
        {{ description }}
      </div>
    </div>

    <div style="display: flex; color: #4e5d56; font-size: 28px;">esfahani.dev</div>
  </div>
</template>
