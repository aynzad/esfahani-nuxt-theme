...
interface RTBlock { type: string, text: string }
interface CodeSlicePrimary {
  code?: RTBlock[]
  lang?: RTBlock[]
}
interface CodeSliceContent { primary: CodeSlicePrimary }
...
const formatted = computed(() => {
  ...
  const lang = langBlock && langBlock.type === 'paragraph' ? langBlock.text : 'javascript'
  return { code: codeBlock.text, lang }
})

const fenced = computed(() =>
  formatted.value ? `\`\`\`${formatted.value.lang}\n${formatted.value.code}\n\`\`\`` : '',
)
</script>
