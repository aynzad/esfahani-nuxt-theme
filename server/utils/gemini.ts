...
const cacheKey = (text: string) => `gemini:fa:${createHash('sha1').update(text).digest('hex')}`

const translationSchema = z.object({
  translations: z
    .array(z.string())
    .describe('Persian translations, in the same order and with the same count as the input array'),
})

// Throws on any failure so the caller can fall back to the original English text.
async function callGemini(texts: string[], { apiKey, model }: TranslateOptions): Promise<string[]> {
  ...
  if (output.translations.length !== texts.length) {
    throw new Error('Gemini returned a different number of translations than requested')
  }
  return output.translations
}

// Caches per source string; empty strings pass through, and any failure falls back to English.
export async function translateToFarsi(texts: string[], options: TranslateOptions): Promise<string[]> {
  ...
  if (misses.length > 0) {
    try {
      ...
      await Promise.all(misses.map(async (text, index) => {
  ...
  return texts.map(text => (text.trim() ? resolved.get(text) ?? text : text))
}
